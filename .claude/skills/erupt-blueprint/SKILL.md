...
2. **章节节奏**：点阵标题行 + 内容行（reveal），严格交替
3. **步骤区块**：用幽灵序号装饰（5.3），不用普通数字列表
4. **按钮**：描边风格，默认无实心背景
5. **区域边界标注（`.ticks`）**：每个 `.r.g-bg` 加 `.ticks`，首个英雄行除外；最后一个标题行加 `only-before`；`.fr` 末尾放空 `<div class="ticks"></div>` 收口
6. **reveal 脚本**：页面底部附 `IntersectionObserver` 初始化代码
7. **装饰上限**：点阵背景 + 区域边界标注 + 出血线（`.bleed-lines`，每屏最多一处），三者之外不引入新装饰

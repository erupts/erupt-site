...
### 5.2 区域边界标注（Section Markers）

在页面**中间区段**的章节边界处（不含页面顶部和底部），可以在行的顶部边框左右两侧加入小型橙红色几何标注，表达"此处是一个可测量区域的边界"。

形式可以根据页面气质选择：
- 朝内的实心三角形（最经典，直接来自 vite.dev）
- 短横线 + 方形端点（更现代感）
- L 形双线角（更精密感）

约束：橙红色、左右对称、尺寸 ≤ 10px、只在中间章节边界使用。
